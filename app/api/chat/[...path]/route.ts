...
export async function GET(request:Request){
 try{
  ...
  if(part[0]==='bootstrap'){
   ...
   const rooms=await db.prepare(`SELECT r.id,r.kind,r.name,r.topic,r.created_at AS createdAt,
...
export async function POST(request:Request){
 try{
  ...
  if(part[0]==='channels'){
   ...
   const saved=await db.prepare('SELECT * FROM rooms WHERE id=?').bind(id).first();if(!saved||saved.creator!==user.id||saved.name!==name||saved.topic!==topic||saved.kind!=='channel')throw new ChatError(409,'作成IDが重複しました。');return json({id},201);
  }
  if(part[0]==='dms'){
  ...
  if(part[0]==='rooms'&&part[2]==='messages'){
   ...
   await db.prepare('INSERT OR IGNORE INTO messages(id,room_id,sender_id,client_id,body,created_at) VALUES(?,?,?,?,?,?)').bind(crypto.randomUUID(),roomId,user.id,clientId,text,Date.now()).run();
