...
import { Hash, MessageSquare, Plus, ChevronDown, ArrowUp, Menu, X, Users, LogOut, LockKeyhole, WifiOff, CircleHelp, Check, ArrowDown, LoaderCircle } from 'lucide-react';

type Person={id:string;name:string};
type Room={id:string;kind:'channel'|'dm';name:string;topic:string;peerName:string|null;lastSeq:number};
...
export default function Chat(){
 ...
 const [modal,setModal]=useState<'channel'|'dm'|'members'|'help'|null>(null);const [formName,setFormName]=useState('');const [topic,setTopic]=useState('');
 ...
 const room=data?.rooms.find(r=>r.id===active);const messages=cache[active]||[];const draft=drafts[active]||'';
 ...
 const refresh=useCallback(async(signal?:AbortSignal)=>{
  const next=await api<Bootstrap>('bootstrap',undefined,signal);
 ...
 useEffect(()=>{if(modal){channelOperation.current=null;setFormError('');setFormName('');setTopic('');dialog.current?.showModal();}else dialog.current?.close();},[modal]);
 async function send(){
 ...
 return <div className="workspace">
  ...
  <main className="chat-main">
   ...
   <header className="conversation-header"><div className="conversation-title"><span className="conversation-icon">{room?.kind==='dm'?<LockKeyhole size={24}/>:<Hash size={26}/>}</span><div><h1>{room?roomName(room):'会話を読み込み中'}</h1><p>{room?.kind==='dm'?'この会話は、あなたと相手だけに表示されます。':room?.topic||'チームの会話をここから。'}</p></div></div>{room?.kind!=='dm'&&<button className="members-button" aria-label="メンバー一覧" onClick={()=>setModal('members')}><Users size={17}/><span>{data.people.length} 人のメンバー</span></button>}</header>
   ...
   <div className="composer-area">{newBelow&&<button className="new-messages" onClick={()=>{nearBottom.current=true;end.current?.scrollIntoView({behavior:'smooth'});setNewBelow(false)}}><ArrowDown size={15}/>新しいメッセージ</button>}
    ...
    <form className="composer" onSubmit={e=>{e.preventDefault();void send()}}><label htmlFor="message-input">{room?.kind==='dm'?`${roomName(room)}さんにメッセージ`:`# ${room?.name||'general'} にメッセージ`}</label>
  ...
  <dialog ref={dialog} onCancel={()=>setModal(null)} onClose={()=>setModal(null)} onClick={e=>{if(e.target===dialog.current)setModal(null)}}>
   <div className="dialog-content"><div className="dialog-heading"><h2>{modal==='channel'?'チャンネルを作成':modal==='dm'?'メッセージを始める':modal==='members'?'ワークスペースのメンバー':'Relayの使い方'}</h2><button className="icon-button" aria-label="ダイアログを閉じる" onClick={()=>setModal(null)}><X size={20}/></button></div>
    {modal==='channel'&&<form onSubmit={e=>{e.preventDefault();void create()}}><p>話題ごとに、会話の場所をつくりましょう。</p><label htmlFor="channel-name">チャンネル名</label><input id="channel-name" value={formName} onChange={e=>setFormName(e.target.value)} placeholder="例：project-launch" required maxLength={40} autoFocus/><label htmlFor="channel-topic">説明 <span>（任意）</span></label><input id="channel-topic" value={topic} onChange={e=>setTopic(e.target.value)} placeholder="どんな話題を共有しますか？" maxLength={200}/><p className="field-hint">このワークスペースのメンバー全員が閲覧できます。</p><button className="primary" disabled={busy||!formName.trim()}>{busy?'作成中…':'チャンネルを作成'}</button></form>}
    {(modal==='dm'||modal==='members')&&<><p>{modal==='dm'?'同じワークスペースのメンバーを選んでください。':'このワークスペースにログインしたメンバーです。'}</p>{modal==='dm'&&<input aria-label="メンバーを検索" placeholder="名前で絞り込み…" value={formName} onChange={e=>setFormName(e.target.value)}/>}<div className="people-list">{data.people.filter(p=>(modal==='members'||p.id!==data.user.id)&&p.name.toLowerCase().includes(formName.toLowerCase())).map(p=><button key={p.id} disabled={busy||p.id===data.user.id} className="person-row" onClick={()=>void create(p.id)}><Avatar name={p.name}/><span>{p.name}{p.id===data.user.id?'（自分）':''}</span>{p.id!==data.user.id&&<MessageSquare size={18}/>}</button>)}</div>{data.people.length===1&&<div className="empty-members"><Users size={24}/><h3>仲間が参加すると、DMを始められます</h3><p>サイトの共有設定でメンバーにアクセス権を付与してください。ログインしたメンバーがここに表示されます。</p></div>}</>}
    {modal==='help'&&<div className="help-copy"><h3><Hash size={18}/>チャンネルで共有</h3><p>左の一覧で話題を選び、下の入力欄から投稿できます。「＋」で新しいチャンネルも作成できます。</p><h3><MessageSquare size={18}/>DMで個別に相談</h3><p>「メッセージを始める」から相手を選びます。内容は参加している2人だけに表示されます。</p><h3><Users size={18}/>仲間を迎える</h3><p>サイトの共有設定でアクセス権を付与し、URLを伝えてください。相手がChatGPTでログインするとメンバー一覧に表示されます。</p><p className="field-hint">通信が切れたときも、送信に失敗した本文は入力欄に残ります。接続後、同じ内容を再送できます。</p></div>}
